...
polybench-$kernel-\${TARGET_NAME}: $kernel.c $kernel.h
	\${VERBOSE} \${CC} -o polybench-$kernel-\${TARGET_NAME} $kernel.c \${CFLAGS} -I. -I$utilityDir $utilityDir/polybench.c \${EXTRA_FLAGS}

clean:
	@ rm -f polybench-$kernel-\${TARGET_NAME}
